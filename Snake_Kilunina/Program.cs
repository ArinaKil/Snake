        ...
        public static void Receiver()
        {
            ...
            try
            {
                ...
                while (true) {
                    ...
                    {
                        int IdPlayer = -1;
                        IdPlayer = remoteIPAddres.FindIndex(x => x.IpAddress == User.IpAddress &&
                            x.Port == User.Port);
                        if (IdPlayer != -1)
                        {
                            if (dataMessage[0] == "Up" &&
        ...
        public static void Timer()
        {
            while (true)
            {
                ...
                if (RemoteSnakes.Count > 0)
                {
                    ...
                    {
                        ViewModelUserSettings User = remoteIPAddres.Find(x => x.IdSnake == DeadSnake.IdSnake);

                        Console.ForegroundColor = ConsoleColor.Yellow;
                        Console.WriteLine($"Отключил пользователя: {User.IdSnake}:{User.Port}");
                        remoteIPAddres.Remove(User);
                    }
